...
const moveWaterMark = (event: MouseEvent|TouchEvent) => {
    const inputX = event instanceof MouseEvent ? event.clientX : event.touches[0].clientX;
    const inputY = event instanceof MouseEvent ? event.clientY : event.touches[0].clientY;
    ...
    if (
        x >= 0 && x <= canvas.value!.width - watermark.size.width && 
        y >= 0 && y <= canvas.value!.height - watermark.size.height
    ) {
...
const drawWatermark = ()=>{
    ...
    watermark.size.width = watermarkImg.width * watermark.scale * canvasInfo.scale;
    watermark.size.height = watermarkImg.height * watermark.scale * canvasInfo.scale;
    ctx!.globalAlpha = watermark.alpha;
    ctx!.drawImage(watermarkImg, watermark.position.x, watermark.position.y, watermark.size.width, watermark.size.height);
    ctx!.globalAlpha = 1;
}

watch(()=>canvas.value,async (value)=>{
    if(!value) return;
    
    const watermarkDailog = useTemplateRef("watermark-dialog");
    const loading = ElLoading.service({target: <HTMLElement>watermarkDailog.value, text: '加载中...'});
    ...
    try{
        ...
        const watermarkLoading =  new Promise((resolve)=>watermarkImg.onload = resolve);
...
watch(
    ()=>[watermark.alpha,watermark.scale,watermark.position.x,watermark.position.y],
    ()=>drawWatermark()
)
...
<template>
    ...
    <div>
        ...
        <div style="margin: 0.2rem 1rem;">
            <strong>水印透明度</strong>
            <el-slider v-model="watermark.alpha" :min="0.3" :max="0.7" :step="0.01" :show-tooltip="false"/>
            <strong>水印大小</strong>
            <el-slider v-model="watermark.scale" :min="0.5" :max="1.5" :step="0.1" :show-tooltip="false"/>
        </div>
    </div>
    <template #footer>
