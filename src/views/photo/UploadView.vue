...
const uploadFormRules = reactive<FormRules<UploadFormInfo>>({
  reg: [{ required: true, message: "请输入注册号/机身编号", trigger: "blur" }],
  msn: [
    {
      message: "请输入制造商序列号",
      trigger: "blur",
      validator: () => {
        if (elemStatus.msn) return true;
        return uploadFormData.msn.length !== 0;
      },
    },
  ],
  airportId: [{ required: true, message: "请选择机场", trigger: "blur" }],
  airlineId: [
...
watch(
  ...
  async (newValue: PhotoType[], oldValue: PhotoType[]) => {
...
async function AutoFill() {
  ...
  const infoReq = new ServerRequest(
    "GET",
    `/aircraft?reg=${uploadFormData.reg}`,
  );
  infoReq.success = () => {
...
const readExifDate = async (file: UploadFile) => {
  ...
  try {
    ...
    uploadFormData.date = isNaN(exif.DateOriginal.getTime())
      ? ""
      : exif.DateOriginal.toISOString().split("T")[0];
    ...
    <div class="intro-box">
      ...
      <div>
        ...
        <div style="padding: 0.2rem 0">
          <el-button type="primary" @click="elemStatus.airportPanel = true"
            >添加机场</el-button
          >
          <el-button type="primary" @click="elemStatus.airlinePanel = true"
            >添加航司/运营人</el-button
          >
          <el-button type="primary" @click="elemStatus.airtypePanel = true"
            >添加机型</el-button
          >
        </div>
        进行补充。
      ...
      <p>
        TOGAPhotos
        <strong>可能在未事先通知您的情况下</strong
        >，将您上传的图片发布在TOGAPhotos的社交媒体账号上（遵循<a
          href="https://creativecommons.org/licenses/by-nc-nd/4.0/deed.zh-Hans"
          >CC BY-NC-ND 4.0协议</a
        >）。其余情况下TOGAPhotos将在事前与您取得联系。
      </p>
      <h4>我们不接受"云图"</h4>
      <p>
        TOGAPhotos只接受用户自己拍摄的图片，<strong>不接受任何形式的"云图"</strong>。
      </p>
    </div>
    <div id="upload-form">
      <el-form
      ...
      >
        ...
        <el-form-item label="注册号/机身编号" prop="reg">
          <el-input v-model="uploadFormData.reg" :disabled="elemStatus.reg" />
          <div class="remark">如果无注册号，请使用NO-REG</div>
          <el-button type="primary" style="width: 100%" @click="AutoFill"
            >自动填充</el-button
          >
        </el-form-item>

        <el-form-item label="制造商序列号" prop="msn">
          <el-input v-model="uploadFormData.msn" :disabled="elemStatus.msn" />
        </el-form-item>
        ...
        <el-form-item label="拍摄位置" prop="airportId">
          <AirportSelect v-model="uploadFormData.airportId" />
        </el-form-item>
        ...
        <el-form-item label="给审图员的留言" prop="message">
          <el-input
            type="textarea"
            :rows="2"
            v-model="uploadFormData.message"
          />
        </el-form-item>

        <el-form-item label="队列选择">
          <el-select v-model="uploadFormData.queue">
            <el-option :label="elemStatus.normQueueText" value="NORM" />
            <el-option
              :label="elemStatus.prioQueueText"
              value="PRIO"
